...
tags:
  - tech
  - datadog
description: "📋 Tổng quan Lab
Mục
Chi tiết
App mẫu
Storedog — ứng dụng e-commerce chạy trên Docker
Microservices
store-frontend, store-backend, store-discounts, store-ads
Service tập trung
store-discounts
Tags quan trọng
env:quickstart-course, service:store-disco"
---
...
1. 📊 D**ashboards**— Khám phá và lọc dữ liệu dashboard
2. 📝 L**ogs**— Tìm kiếm, lọc và trực quan hóa log
3. 🗂️ S**oftware Catalog**— Xem thông tin tổng quan về service
4. 🔔 M**onitors**— Đọc hiểu và diễn giải alert/monitor

---

## 💡 Khái niệm cốt lõi: Tags

> Tags là các cặp key:value dùng để gắn metadata cho dữ liệu trong Datadog.

**Công dụng của tags:**
- 🔍 S**earch & Filter**— Lọc dữ liệu theo tag
- 📦 G**roup**— Nhóm dữ liệu liên quan
- 🔗 C**orrelate**— Datadog tự động liên kết dữ liệu có cùng tag

**Tags dùng trong lab này:**
| Tag | Ý nghĩa |
|:--------------------------|:-----------------------------------------------|
| `env:quickstart-course` | Dữ liệu từ môi trường lab quickstart |
| `service:store-discounts` | Dữ liệu từ service store-discounts |
...
> Menu chính → hover Dashboards → chọn Dashboard List

**Bước 2 — Xem danh sách dashboard**
Danh sách gồm:
- Các OOTB dashboards (tự động cài theo cấu hình Storedog)
- 1 custom dashboard: **Storedog 2.0** **Bước 3 — Mở Storedog 2.0**
Dashboard có **3 nhóm widget:**
- **Overall Performance**
- **Infrastructure & Network**
- **Databases** **Bước 4 — Tương tác với widget**
...
> ⚠️ Quan sát: Sau khi lọc, widgets SLO: Python service high errors, Monitors Status, và Error Rate % cho thấy store-discounts có error rate cao → cần điều tra.

**Bước 6 — Đánh dấu yêu thích (★)**

> Click icon ★ cạnh tiêu đề dashboard → thêm vào Starred Dashboards

**Bước 7 — Khám phá Docker - Overview dashboard**

> Trong widget Overall Performance → click link Docker - Overview
...
> ✅ Kết quả: Sức khỏe container store-discounts tốt, không ảnh hưởng đến performance của service.

**Bước 9 — Đánh dấu Docker - Overview và kiểm tra Starred**
...
- **Log list** (giữa): danh sách logs**Bước 3 — Lọc log của store-discounts**
...
- **Tags** gắn với log
- **Nội dung** log
- Tabs: **Fields & Attributes** \|**Trace** \|**Metrics** \|**Processes** **Bước 5 — Lọc log lỗi (Error)**
...
> 💡 Tip: Dùng phím ↑↓ để chuyển nhanh giữa các log trong list

**Bước 7 — Xóa filter status:error**
...
> ℹ️ Timeseries cho thấy số lượng logs nhưng không hiển thị %.

**Bước 9 — Chuyển sang Pie Chart để xem %**
...
> Menu chính → Automation > Internal Developer Portal > Software Catalog
...
> Logs > Search & Analytics > Explorer → click Views (góc trên trái) → chọn view đã lưu

### Bài 2: Tìm Manage Monitors và lọc theo store-discounts

> Monitoring > Monitor List → Facets → Service → chọn store-discounts
...
👉 D[atadog Foundation
[Datadog Quick Start](anytype://object?objectId=bafyreigilgjh4zjvvwkzqlsvorfmkdrjy2xl3hzp36eraxafod2xujdhbi&spaceId=bafyreihkelp7ljuvnh2znrup2r2z4q33qbtegpe6fzs5grmwpfiqz2q4e4.2s07itt5u9svl)
